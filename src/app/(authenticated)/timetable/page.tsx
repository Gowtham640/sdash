...
export default function TimetablePage() {
    ...
    try {
      ...
      if (!hasValidCache || forceRefresh) {
        ...
        if (result.data && typeof result.data === 'object' && 'calendar' in result.data) {
          ...
          if (calendarArray) {
            console.log('[Timetable] ✅ Calendar data loaded');
          } else {
        ...
        if (timetableDataObj && timetableDataObj.timetable) {
          ...
          console.log('[Timetable] Loaded timetable successfully');
        } else {
        ...
        if (cachedUnified && typeof cachedUnified === 'object' && cachedUnified !== null && 'data' in cachedUnified) {
          ...
          if (unifiedData && typeof unifiedData === 'object' && 'calendar' in unifiedData) {
            ...
            if (calendarArray) {
              console.log('[Timetable] ✅ Calendar data loaded from unified cache');
            }
          }
        } else {
          ...
          try {
            ...
            if (calendarResult.success && calendarResult.data?.calendar) {
              ...
              if (calendarArray) {
                console.log('[Timetable] ✅ Calendar data loaded from fetch');
              }
            }
          } catch (calendarErr) {
  ...
  if (loading) {
    return (
      <div className="h-full p-8 flex flex-col gap-8">
        <div className="w-full h-full bg-white/10 border border-white/20 rounded-3xl text-white text-3xl font-sora flex flex-col gap-6 justify-center items-center">
  ...
  if (error) {
    return (
      <div className="h-full p-8 flex flex-col gap-8">
        <div className="w-full h-full bg-white/10 border border-white/20 rounded-3xl text-white text-3xl font-sora flex flex-col gap-10 justify-center items-center">
          <div className="text-white text-4xl font-sora font-bold">Timetable</div>
  ...
  if (!rawTimetableData || !timetableData || timetableData.length === 0) {
    return (
      <div className="h-full p-8 flex flex-col gap-8">
        ...
        <div className="w-full h-auto bg-white/10 border border-white/20 rounded-3xl text-white text-xs sm:text-sm md:text-base lg:text-lg font-sora flex flex-col gap-6 justify-center items-center p-8">
  ...
  return (
    <div className="h-full p-8 flex flex-col gap-8">
      ...
      <div className="w-full h-auto bg-white/10 border border-white/20 rounded-3xl text-white text-xs sm:text-sm md:text-base lg:text-lg font-sora overflow-hidden">
        <div className="h-full overflow-auto">
          <table className="w-full h-full border-collapse">
            <thead className="sticky top-0 bg-black/50 backdrop-blur-sm z-10">
              <tr>
                <th className="border border-white/30 bg-white/20 p-2 sm:p-2.5 md:p-3 lg:p-3 text-center font-bold min-w-[70px] sm:min-w-[80px] md:min-w-[85px] lg:min-w-[90px] text-[10px] sm:text-xs md:text-sm lg:text-base">
                  Time
                </th>
                {days.map((day) => (
                  <th key={day} className="border border-white/30 bg-white/20 p-2 sm:p-2.5 md:p-3 lg:p-3 text-center font-bold min-w-[80px] sm:min-w-[90px] md:min-w-[100px] lg:min-w-[110px] text-[10px] sm:text-xs md:text-sm lg:text-base">
                    {day}
                  </th>
                ))}
              </tr>
