...
export default function LiquidEther({
  ...
  useEffect(() => {
    if (!mountRef.current) {
      console.error('LiquidEther: mountRef.current is null');
      return;
    }
    ...
    class CommonClass {
      ...
      init(container: HTMLElement) {
        if (!container) {
          console.error('Common: container is null');
          return;
        }
    ...
    class AutoDriver {
      ...
      update() {
        ...
        // Always keep auto driver active for continuous animation
        if (!this.active) {
          this.active = true;
        ...
        // Reduce intensity when user is actively interacting
        const isUserActive = idle < this.resumeDelay;
        const intensity = isUserActive ? 0.3 : 1.0; // Lower intensity when user is active
        ...
        const step = this.speed * dtSec * ramp * intensity;
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      init() {
        if (!Common.renderer) {
          console.error('WebGLManager: Common.renderer is null');
          return;
        }
        if (!this.props.$wrapper) {
          console.error('WebGLManager: $wrapper is null');
          return;
        }
